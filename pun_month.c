// Program to compute monthly PUN band averages and write results to Google Sheets
...
static void get_easter_monday(int year, int *out_m, int *out_d) {
    ...
    // Easter Monday is Easter Day + 1 day
    if (month_e == 3) {
        if (day_e < 31) {
            *out_m = 3;
            *out_d = day_e + 1;
...
static int is_festivo(struct tm *tm) {
    ...
    // Dynamic check for Easter Monday
    int em_m = 0, em_d = 0;
...
// Update Google Sheet row using Google Sheets API v4 (PUT request)
static int update_google_sheet(const char *token, const char *aaaamm, int row_index, double f0, double f1, double f2, double f3) {
    CURL *curl = NULL;
    ...
    // Construct API endpoint URL for target row
    snprintf(url, sizeof(url),
        "https://sheets.googleapis.com/v4/spreadsheets/%s/values/%s!A%d:E%d?valueInputOption=USER_ENTERED",
    ...
    snprintf(json_payload, sizeof(json_payload),
        ...
        "]]"
        "}",
        SHEET_NAME, row_index, row_index, aaaamm, f0, f1, f2, f3);
...
int main(int argc, char *argv[]) {
    ...
    // Calculate row index in Google Sheet (Row 2 = 202510)
    ...
    // Set application timezone to Europe/Rome
    setenv("TZ", "Europe/Rome", 1);
    tzset();

    // Define monthly interval boundaries in Italian local time
    struct tm start_tm;
    ...
    // Initialize MySQL connection using setup_energy variables
    ...
    // Read Google OAuth2 access token
    ...
    // Update target Google Sheet row
    if (!update_google_sheet(access_token, argv[1], row_index, f0, f1, f2, f3)) {
        fprintf(stderr, "Error: failed to update Google Sheet\n");
        curl_global_cleanup();
        return 1;
    }

    printf("OK: Updated Sheet row %d for %s | F0: %.5f | F1: %.5f | F2: %.5f | F3: %.5f\n",
        row_index, argv[1], f0, f1, f2, f3);
